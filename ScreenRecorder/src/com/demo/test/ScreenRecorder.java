...
public class ScreenRecorder {
    private static final String TAG = "xshx.ScreenRecorder";
    ...
    private void signalEndOfStream() {
        Log.d(TAG, "signalEndOfStream: ", new Throwable());
        ...
        VideoEncoder.Callback callback = new VideoEncoder.Callback() {
            ...
            public void onOutputBufferAvailable(BaseEncoder codec, int index, MediaCodec.BufferInfo info) {
                ...
                try {
                    ...
                    Log.d(TAG, "Muxer encountered an error! ", e);
            ...
            public void onError(Encoder codec, Exception e) {
                ranIntoError = true;
                Log.d(TAG, "VideoEncoder ran into an error! ", e);
                Message.obtain(mHandler, MSG_ERROR, e).sendToTarget();
            }
    ...
    private void prepareAudioEncoder() throws IOException {
        ...
        AudioEncoder.Callback callback = new AudioEncoder.Callback() {
            ...
            @Override
            public void onOutputBufferAvailable(BaseEncoder codec, int index, MediaCodec.BufferInfo info) {
                Log.d(TAG, "[" + Thread.currentThread().getId() + "] AudioEncoder output buffer available: index=" + index);
                try {
                    muxAudio(index, info);
                } catch (Exception e) {
                    Log.d(TAG, "Muxer encountered an error! ", e);
            ...
            public void onError(Encoder codec, Exception e) {
                ranIntoError = true;
                Log.d(TAG, "MicRecorder ran into an error! ", e);
                Message.obtain(mHandler, MSG_ERROR, e).sendToTarget();
            }
